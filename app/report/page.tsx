        ...
        <div className="report-section no-print admin-only"><div className="report-section-head"><div><span>ASSUMPTIONS</span><br /><strong>사업비·운영·금융 입력</strong></div></div>
          ...
          {structurePolicy?.policy.usesExitCapRate && (
            <div className="report-note" style={{ marginTop: 8 }}>
              이 구조({structurePolicy.policy.structureName})는 기간 말 잔존가치를 Exit Cap Rate로 산정합니다.
              시장 기준값이 DB에 아직 없어 위 칸에 직접 입력해야 IRR에 반영되고, 비워두면 잔존가 0(보수적 가정)으로 계산됩니다.
            </div>
          )}
          ...
          <div className="report-note" style={{ marginTop: 10 }}>
            ...
            {structurePolicy?.policy.usesExitCapRate && (
              <>
                {" "}· 잔존가치 {assumptions.exitCapRatePct
                  ? `Exit Cap Rate ${assumptions.exitCapRatePct}% 적용`
                  : "미입력 — 잔존가 0으로 계산"}
              </>
            )}
          </div>
